...
class ConfigParser:
    ...
    def get_interactions_data(self):
        ...
        print("🔍 DEBUG: Full YAML Config:", self.cfg)  # 🔥 Print full YAML config
        interactions_data = self.cfg.get("interactions", {})

        print("🔍 DEBUG: interactions_data =", interactions_data)  # 🔥 Print what is retrieved

        # Ensure it is a dictionary
        if not isinstance(interactions_data, dict):
            raise TypeError(f"Expected a dictionary for interactions data, but got {type(interactions_data)}")

        if "type" not in interactions_data:
            raise KeyError("Missing 'type' key in interactions data")
